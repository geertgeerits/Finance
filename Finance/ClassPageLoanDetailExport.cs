    ...
    internal class ClassPageLoanDetailExport
    {
        ...
        public static void ExportDetailLoanCSV(int nNumberPeriods, string cCurrency, string cDocTitle, string cFileName)
        {
            ...
            try
            {
                ...
                // Data loan.
                sw.WriteLine($"{lblInterestRateText} {entInterestRateText} %");
                double nCapitalInitial = Convert.ToDouble(entCapitalInitialText);
                sw.WriteLine($"{lblCapitalInitialText} {Globals.RoundDoubleToNumDecimals(ref nCapitalInitial, Convert.ToInt32(Globals.cNumDecimalDigits), "N")} {cCurrency}");
                sw.WriteLine($"{lblDurationYearsText} {entDurationYearsText}");
                sw.WriteLine($"{lblPeriodsYearText} {entPeriodsYearText}");
                sw.WriteLine("");
        ...
        public static void ExportDetailLoanHTML(int nNumberPeriods, string cCurrency, string cDocTitle, string cFileName)
        {
            ...
            try
            {
                ...
                // Data loan.
                sw.WriteLine($"<p>{lblInterestRateText} {entInterestRateText} %<br>");
                double nCapitalInitial = Convert.ToDouble(entCapitalInitialText);
                sw.WriteLine($"{lblCapitalInitialText} {Globals.RoundDoubleToNumDecimals(ref nCapitalInitial, Convert.ToInt32(Globals.cNumDecimalDigits), "N")} {cCurrency}<br>");
                sw.WriteLine($"{lblDurationYearsText} {entDurationYearsText}<br>");
                sw.WriteLine($"{lblPeriodsYearText} {entPeriodsYearText}</p>");
        ...
        public static void ExportDetailLoanPDF(int nNumberPeriods, string cCurrency, string cDocTitle, string cFileName)
        {
            ...
            // Portrait.
            if (entCapitalInitialText.Length < 14)
            {
            ...
            try
            {
                ...
                for (nPagNo = 1; nPagNo < nNumPages + 1; nPagNo++)
                {
                    ...
                    {
                        nHeight -= 12;
                        cText = $"BT /F1 10 Tf {cHorPointsBeg} {Convert.ToString(nHeight)} Td ({ReplaceCharacters(lblInterestRateText)} {entInterestRateText} %)Tj ET";
                        cTextLines[nTextLine] = cText;
                        nTextLine++;

                        double nCapitalInitial = Convert.ToDouble(entCapitalInitialText);
                        nHeight -= 15;
                        cText = $"BT /F1 10 Tf {cHorPointsBeg} {Convert.ToString(nHeight)} Td ({ReplaceCharacters(lblCapitalInitialText)} {Globals.RoundDoubleToNumDecimals(ref nCapitalInitial, nNumDec, "N")} {cCurrency})Tj ET";
                        cTextLines[nTextLine] = cText;
                        nTextLine++;

                        nHeight -= 15;
                        cText = $"BT /F1 10 Tf {cHorPointsBeg} {Convert.ToString(nHeight)} Td ({ReplaceCharacters(lblDurationYearsText)} {entDurationYearsText})Tj ET";
                        cTextLines[nTextLine] = cText;
                        nTextLine++;

                        nHeight -= 15;
                        cText = $"BT /F1 10 Tf {cHorPointsBeg} {Convert.ToString(nHeight)} Td ({ReplaceCharacters(lblPeriodsYearText)} {entPeriodsYearText})Tj ET";
                        cTextLines[nTextLine] = cText;
                        nTextLine++;
